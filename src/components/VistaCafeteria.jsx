...
function VistaCafeteria({ nombreUsuario, avatarConfig, getAvatarUrl }) {
  // --- ESTADOS DEL MAPA 2D ---
  const [posicion, setPosicion] = useState({ x: 50, y: 50 }); // Coordenadas de inicio del avatar
  const [mesaCercana, setMesaCercana] = useState(null);
  const [mesaActiva, setMesaActiva] = useState(null);

  // --- ESTADOS DEL CHAT DE MESA ---
  const [mensaje, setMensaje] = useState("");
  const [chatMesa, setChatMesa] = useState([]);
  const chatRef = useRef(null);

  // Definición de las mesas adaptadas a la nueva imagen cenital
  const mesas = [
  ...
  useEffect(() => {
    ...
    const manejarTeclado = (e) => {
      ...
      if (e.key === 'Enter' && mesaCercana !== null) {
        setMesaActiva(mesaCercana);
      }
    };
  ...
  useEffect(() => {
    let cerca = null;
    mesas.forEach(mesa => {
      const distanciaX = Math.abs(posicion.x - mesa.x);
      const distanciaY = Math.abs(posicion.y - mesa.y);
      
      if (distanciaX < 15 && distanciaY < 15) {
        cerca = mesa.id;
      }
    });
  ...
  // ==========================================
  // RENDER 1: ESTAMOS SENTADOS EN LA MESA (CHAT)
  // ==========================================
  if (mesaActiva !== null) {
      ...
      <div className="max-w-4xl mx-auto h-[650px] flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-[#001a17] border-x border-b border-white/5 flex-1 flex flex-col overflow-hidden rounded-b-3xl shadow-2xl">
          <div className="flex-1 overflow-y-auto p-4 flex flex-col-reverse" ref={chatRef}>
            {chatMesa.map((msg) => (
              <div key={msg.id} className={`flex gap-3 my-2 ${msg.nombre === nombreUsuario ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[70%] p-3 rounded-2xl ${msg.nombre === nombreUsuario ? `bg-${mesaActual.color}-400/20 rounded-tr-none text-right` : 'bg-white/5 rounded-tl-none'}`}>
                  <h4 className={`font-bold text-[11px] uppercase tracking-wider mb-1 ${msg.nombre === nombreUsuario ? `text-${mesaActual.color}-400` : 'text-gray-400'}`}>
                    {msg.nombre}
                  </h4>
            ...
            {chatMesa.length === 0 && (
              <div className="text-center text-gray-500 text-sm mt-10">La mesa está vacía. ¡Rompe el hielo!</div>
            )}
          </div>
  ...
  // ==========================================
  // RENDER 2: MAPA VIRTUAL 2D DE LA CAFETERÍA
  // ==========================================
  return (
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto space-y-4">
      ...
      {/* Contenedor del Mapa 2D con la Nueva Ilustración (Cargada desde public) */}
      <div 
      ...
      >
        ...
        {/* Zonas interactivas superpuestas a las mesas del dibujo */}
        {mesas.map((mesa) => (
          <div 
            key={mesa.id}
            className={`absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-1 transition-all duration-300 ${mesaCercana === mesa.id ? 'scale-110' : 'scale-100 opacity-80'}`}
            style={{ left: `${mesa.x}%`, top: `${mesa.y}%` }}
          >
            {/* Círculo sutil (brillo) para indicar que es interactivo */}
        ...
        <div 
          className="absolute transform -translate-x-1/2 -translate-y-1/2 transition-all duration-75 ease-linear flex flex-col items-center gap-1 z-20"
          style={{ left: `${posicion.x}%`, top: `${posicion.y}%` }}
        >
          <div className="bg-white text-black text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-lg mb-1">
            Tú
          </div>
          <img 
            src={getAvatarUrl(avatarConfig)} 
            alt="Mi Avatar" 
            className={`w-12 h-12 rounded-full border-2 bg-gray-800 shadow-xl ${mesaCercana !== null ? 'border-emerald-400 scale-110' : 'border-white'}`} 
          />
          {/* Sombra proyectada */}
