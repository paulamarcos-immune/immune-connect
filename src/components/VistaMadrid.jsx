...
function VistaMadrid({ setVistaActiva }) {
  // Configuración de las tres cuentas de Instagram para las cajas inferiores
  const cuentasIG = [
    { 
      id: 1, 
      usuario: "@barbygant", 
      link: "https://www.instagram.com/barbygant/"
    },
    { 
      id: 2, 
      usuario: "@planesbrutales", 
      link: "https://www.instagram.com/planesbrutales/"
    },
    { 
      id: 3, 
      usuario: "@madrid_secreto", 
      link: "https://www.instagram.com/madrid_secreto/"
    },
  ];
  ...
  return (
    <div className="animate-in fade-in duration-300 max-w-5xl mx-auto space-y-8 pb-10">
      ...
      {/* SECCIÓN DE CUENTAS RECOMENDADAS (Las 3 cajas básicas) */}
      <div>
        <h3 className="text-xs text-gray-400 uppercase font-bold tracking-widest mb-6 px-2">Cuentas que debes seguir</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {cuentasIG.map((cta) => (
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="group relative h-40 rounded-3xl overflow-hidden shadow-xl border border-white/5 hover:border-yellow-400/30 transition-all duration-300 bg-[#001a17]"
            >
              {/* Usamos unavatar.io para poner la foto de perfil de fondo con blur */}
              <img 
                src={`https://unavatar.io/instagram/${cta.usuario.replace('@','')}`} 
                className="absolute inset-0 w-full h-full object-cover opacity-20 group-hover:opacity-40 transition-opacity blur-sm" 
                alt={cta.usuario}
              />
              
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                {/* Foto de perfil pequeña en el centro */}
                <div className="w-14 h-14 rounded-full border-2 border-white/10 group-hover:border-yellow-400/50 transition-colors overflow-hidden">
                   <img src={`https://unavatar.io/instagram/${cta.usuario.replace('@','')}`} alt="avatar" />
                </div>
                {/* Nombre de usuario */}
                <span className="text-white font-bold tracking-tight text-lg group-hover:text-yellow-400 transition-colors">
                  {cta.usuario}
                </span>
                
                {/* Icono pequeño de Instagram */}
